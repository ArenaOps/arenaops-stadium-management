...
import { useEventsQuery } from "@/features/public/hooks";
import styles from "./EventDiscovery.module.scss";

function EventDiscoverySkeleton() {
  return (
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3" aria-label="Loading events">
      {Array.from({ length: 6 }).map((_, index) => (
        <Card key={`event-skeleton-${index}`} className={styles.eventCard}>
    ...
    <section className={styles.eventDiscoverySection}>
      <div className="container mx-auto px-6 relative z-10">
        <div className={cn("flex flex-col md:flex-row justify-between items-end gap-4", styles.sectionHeader)}>
          ...
          <Button asChild variant="ghost" className="text-[#10b981] hover:text-[#10b981] hover:bg-white/5 group h-12 px-6 rounded-full border border-white/10">
            <Link href="/events">
              View All Events
        ...
        {isLoading && <EventDiscoverySkeleton />}

        {isError && (
          <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-6 text-center">
            <p className="text-sm text-red-200 mb-4">Events failed to load. Please try again.</p>
            <Button type="button" onClick={() => void refetch()} variant="outline" className="border-red-300 text-red-100 hover:bg-red-500/20">
              Retry
            </Button>
          </div>
        )}
        ...
        {!isLoading && !isError && events.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-400 text-lg">No events are live right now.</p>
          </div>
        )}
              ...
              <Card key={event.eventId} className={styles.eventCard}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="pt-4 flex items-baseline gap-1">
                    <span className={styles.priceTag}>Rs. 999+</span>
                    <span className="text-xs text-slate-500 font-medium tracking-wide">/ PERSON</span>
                  </div>
                </CardContent>

                <CardFooter className="pt-2">
                  <Button asChild className={cn("w-full h-12 text-sm font-bold uppercase tracking-widest", styles.bookButton)} variant="outline">
                    <Link href={`/events/${event.eventId}`}>Book Tickets</Link>
                  </Button>
                </CardFooter>
              </Card>
            ))}
