...
/// <summary>
/// Seating Plan (Template) CRUD APIs.
/// 
/// Routes per API docs (04-Api-Documentation.md):
///   POST /api/stadiums/{stadiumId}/seating-plans       → Owner → Create base template
///   GET  /api/stadiums/{stadiumId}/seating-plans        → Any   → List templates for stadium
///   GET  /api/seating-plans/{id}                        → Any   → Full template with sections + landmarks
///   PUT  /api/seating-plans/{id}                        → Owner → Update seating plan
///   DELETE /api/seating-plans/{id}                      → Owner → Delete seating plan
/// </summary>
[ApiController]
[Authorize]
public class SeatingPlanController : ControllerBase
{
    ...
    /// <summary>
    /// List seating plans for a stadium
    /// </summary>
    [HttpGet("api/stadiums/{stadiumId:guid}/seating-plans")]
    ...
    /// <summary>
    /// Create a new seating plan for a stadium (Stadium Owner only)
    /// </summary>
    [HttpPost("api/stadiums/{stadiumId:guid}/seating-plans")]
    [Authorize(Roles = "StadiumOwner,Admin")]
    public async Task<IActionResult> Create(Guid stadiumId, [FromBody] CreateSeatingPlanRequest request, CancellationToken cancellationToken)
    {
        ...
        // Ensure the route stadiumId matches the body
        request.StadiumId = stadiumId;
    ...
    /// <summary>
    /// Get seating plan by ID (full template with sections + landmarks)
    /// </summary>
    [HttpGet("api/seating-plans/{id:guid}")]
    ...
    /// </summary>
    [HttpPut("api/seating-plans/{id:guid}")]
    [Authorize(Roles = "StadiumOwner,Admin")]
    ...
    /// </summary>
    [HttpDelete("api/seating-plans/{id:guid}")]
    [Authorize(Roles = "StadiumOwner,Admin")]
